...
// Per-section list card for the CLASSIC engine editor (BarEditor, Phase D4-E2).
...
//     BarLayout.classicMoveTo/classicMoveGroupTo, so engine and visuals agree),
//   * an empty card is still a valid target (index 0).
// Cards expose the same query API as ZoneEditorCard (containsRootPoint /
// dropIndexAt / insertionGeometry) plus dndTargetKind: "classic" and dndTargetId
...
Rectangle {
    ...
    property bool isClassicEditorCard: true
    readonly property string dndTargetKind: "classic"
    readonly property string dndTargetId: classicCard.listId
    ...
    readonly property string listTitle: classicCard.listId === "available" ? "Available"
        : (classicCard.listId === "center" ? "Center"
        : (classicCard.listId === "right" ? "Right" : "Left"))

    // Fresh model on every classic config change: section items as stored
    // (strings + group arrays) or the catalog ids of the available pool.
    readonly property var chipsModel: classicCard.listId === "available"
        ? BarLayout.classicAvailableModules(classicCard.bar.classic)
        : BarLayout.classicSectionItems(classicCard.bar.classic, classicCard.listId)
    ...
    function containsRootPoint(mx, my) {
        let p = classicCard.mapFromItem(classicCard.bar, mx, my);
        return p.x >= 0 && p.y >= 0 && p.x <= classicCard.width && p.y <= classicCard.height;
    }
    ...
    function dropChips() {
        ...
        let out = [];
        let groupDrag = classicCard.bar.dndBusy && classicCard.bar.dndModuleId === "";
        for (let i = 0; i < kids.length; i++) {
            let c = kids[i];
            if (!c || !c.isClassicItemChip) continue;
            if (groupDrag) {
                if (classicCard.bar.dndSourceClassicList === classicCard.listId && c.chipItemIndex === classicCard.bar.dndSourceItemIndex) continue;
            } else {
                if (classicCard.bar.dndModuleId !== "" && c.chipItemId === classicCard.bar.dndModuleId) continue;
            }
            out.push(c);
        }
        return out;
    ...
    function dropIndexAt(mx, my) {
        if (!classicCard.containsRootPoint(mx, my)) return -1;
        let chips = classicCard.dropChips();
        if (chips.length === 0) return 0;
        let f = chipsFlow.mapFromItem(classicCard.bar, mx, my);
        let rows = [];
    ...
    function groupJoinIndexAt(mx, my) {
        if (classicCard.listId === "available") return -1;
        let b = classicCard.bar;
    ...
    function insertionGeometry() {
        ...
        let spacing = chipsFlow.spacing;
        if (chips.length === 0) {
            let p0 = chipsFlow.mapToItem(classicCard, 0, 0);
            return { x: p0.x + bar.s(1), y: p0.y + bar.s(2), height: bar.s(26) };
        }
        let idx = Math.max(0, Math.min(classicCard.dndInsertIndex, chips.length));
        if (idx < chips.length) {
            let c = chips[idx];
            let p = c.mapToItem(classicCard, 0, 0);
            return { x: p.x - spacing / 2, y: p.y, height: c.height };
        }
        let last = chips[chips.length - 1];
        let pend = last.mapToItem(classicCard, last.width, 0);
        return { x: pend.x + spacing / 2, y: pend.y, height: last.height };
    }

    function updateInsertBar() {
        if (!classicCard.dndActive || classicCard.dndInsertIndex < 0 || classicCard.dndJoinIndex >= 0) {
            insertBar.visible = false;
            return;
        }
        let g = classicCard.insertionGeometry();
        insertBar.visible = true;
        insertBar.x = g.x;
        insertBar.y = g.y;
        insertBar.height = g.height;
    }
    onDndActiveChanged: classicCard.updateInsertBar()
    onDndInsertIndexChanged: classicCard.updateInsertBar()
    onDndJoinIndexChanged: classicCard.updateInsertBar()
    ...
    Column {
        ...
        Row {
            width: parent.width
            spacing: bar.s(6)
            SectionTitle { bar: classicCard.bar; text: classicCard.listTitle }
            EditLabel {
                bar: classicCard.bar
                anchors.verticalCenter: parent.verticalCenter
                text: classicCard.listId === "available" ? "Not on the bar — drag a chip into a section (or onto a group) to add it."
        ...
        EditLabel {
            bar: classicCard.bar
            width: parent.width
            visible: classicCard.chipsModel.length === 0
            text: classicCard.listId === "available" ? "Every catalog module is on the bar."
        ...
        Flow {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    property bool isClassicItemChip: true
                    readonly property bool chipIsGroup: (typeof modelData === "object") && BarLayout.isList(modelData)
                    ...
                    readonly property bool chipHasMinus: !chipRoot.chipIsGroup && classicCard.listId !== "available"

                    // Module metadata of a loose entry (null for tokens).
                    readonly property var looseMod: chipRoot.chipIsGroup ? null : BarLayout.getModule(String(modelData))
                    ...
                    opacity: (bar.dndBusy && (
                        ...
                        || (bar.dndModuleId === "" && bar.dndSourceClassicList === classicCard.listId && chipRoot.chipItemIndex === bar.dndSourceItemIndex))
                    ) ? 0.35 : 1.0
                        ...
                        Row {
                            ...
                            Repeater {
                                ...
                                delegate: Rectangle {
                                    ...
                                    readonly property var memberMod: BarLayout.getModule(memberId)
                                    ...
                                    MouseArea {
                                        ...
                                        onPositionChanged: mouse => {
                                            ...
                                            bar.startClassicDnd(classicCard.listId, memberId, -1);
                                            if (!bar.dndBusy) {
                                    ...
                                    Rectangle {
                                        ...
                                        MouseArea {
                                            anchors.fill: parent
                                            cursorShape: Qt.PointingHandCursor
                                            onClicked: classicCard.bar.classicRemoveModule(memberId)
                                        }
                                    }
                                }
                            }
                    ...
                    MouseArea {
                        ...
                        onPositionChanged: mouse => {
                            ...
                            bar.startClassicDnd(classicCard.listId, chipRoot.chipItemId, -1);
                            if (!bar.dndBusy) {
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            anchors.fill: parent
                            cursorShape: Qt.PointingHandCursor
                            onClicked: classicCard.bar.classicRemoveModule(chipRoot.chipItemId)
                        }
                    }
                    ...
                    MouseArea {
                        ...
                        onPositionChanged: mouse => {
                            ...
                            bar.startClassicDnd(classicCard.listId, "", chipRoot.chipItemIndex);
                            if (!bar.dndBusy) {
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            anchors.fill: parent
                            cursorShape: Qt.PointingHandCursor
                            onClicked: classicCard.bar.classicUngroupAt(classicCard.listId, chipRoot.chipItemIndex)
                        }
                    }
                    ...
                    Rectangle {
                        ...
                        radius: bar.s(10)
                        visible: chipRoot.chipIsGroup && classicCard.dndActive && classicCard.dndJoinIndex === chipRoot.chipItemIndex
                        color: "transparent"
