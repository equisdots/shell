...
// ═══════════════════════════════════════════════════════════════════════════
// ENGINES CATALOG (Engine page) — data-driven: a new bar engine only needs an
// entry here, its component and its WindowRegistry entry; the panel renders
// whatever this list contains.
// ═══════════════════════════════════════════════════════════════════════════
var ENGINES = [
    { id: "bar", label: "Bar", icon: "\u{f0b2f}", accentRole: "mauve",
...
function engines() {
    ...
    for (let i = 0; i < ENGINES.length; i++) {
        let e = ENGINES[i], o = { capabilities: {} };
        for (let k in e) o[k] = e[k];
        for (let k in e.capabilities) o.capabilities[k] = e.capabilities[k];
        out.push(o);
    }
    return out;
}
function engineCapabilityList(e) {
    let c = (e && e.capabilities) || {}, out = [];
    if (c.zones) out.push("zones");
    if (c.autohide) out.push("autohide");
    if (c.distinctPills) out.push("distinct pills");
    if (c.fillStyle) out.push("fill styles");
    if (c.hotSwitch) out.push("hot switch");
    return out.join(" · ");
}
...
var CLASSIC_TOKEN_IDS = ["center", "centerbox"];
...
function classicTokenModules(id) {
    ...
    if (CLASSIC_TOKEN_IDS.indexOf(id) !== -1) return ["time", "weather"];
...
// may hold loose strings (catalog module ids or CLASSIC_TOKEN_IDS tokens) and
...
// through isFillStyle().
// Visual flags for the CLASSIC bar styles (its own map — deliberately NOT the
...
var CLASSIC_STYLE_FLAGS = {
    "modular": { pillBg: true,  pillSolid: false, barBg: false },
    "solid":   { pillBg: false, barBg: true },
    "fill":    { pillBg: false, barBg: true, edgeGap: 0 }
};
function classicStyleFlags(style) {
    let preset = CLASSIC_STYLE_FLAGS.hasOwnProperty(style) ? CLASSIC_STYLE_FLAGS[style] : null;
    let out = {};
...
function classicMoveTo(cfg, id, toList, insertIndex) {
    ...
    if (!getModule(id) && CLASSIC_TOKEN_IDS.indexOf(id) === -1) return cfg; // unknown id
