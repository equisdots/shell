...
//
// The active palette is controlled by settings.json: "bar": { "palette": "x" }
...
Item {
    ...
    // (e.g. a BarEditor commit that did not actually change the value) are
    ...
    // --- live watcher on the active palette file (Phase-T BarEditor edits) ---------
    ...
    // did not actually change, so the BarEditor <-> Colors refresh loop is
