    ...
    delegate: Component {
        PanelWindow {
            ...
            function syncBarConfig() {
                ...
                barWindow.dateFormat = (typeof barConfig.dateFormat === "string" && barConfig.dateFormat !== "") ? barConfig.dateFormat : "dddd, MMMM dd";
            ...
            FileView {
                id: wsWatcher
                path: paths.getRunDir("workspaces") + "/workspaces.json"
                watchChanges: true
                onFileChanged: {
            ...
            Process {
                ...
                running: true
                command: ["bash", "-c", "bash ~/.config/hypr/scripts/quickshell/ui/bar/popups/music/music_info.sh | tee " + paths.getRunDir("music") + "/music_info.json"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process {
                id: weatherPoller
                command: ["bash", "-c", `
                    echo "$(~/.config/hypr/scripts/quickshell/ui/bar/popups/calendar/weather.sh --current-icon)"
                    echo "$(~/.config/hypr/scripts/quickshell/ui/bar/popups/calendar/weather.sh --current-temp)"
                    echo "$(~/.config/hypr/scripts/quickshell/ui/bar/popups/calendar/weather.sh --current-hex)"
                `]
                stdout: StdioCollector {
                    onStreamFinished: {
            ...
            Timer {
                ...
                onTriggered: {
                    ...
                    barWindow.fullDateStr = Qt.formatDateTime(d, barWindow.dateFormat);
