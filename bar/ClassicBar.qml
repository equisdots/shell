...
//                            Row (horizontal bar) or Column (vertical bar) of
...
Item {
    ...
    required property var classicConfig
    ...
    // The strip band box. The host's clickthrough mask (barWindow.mask)
    ...
    property alias classicContentArea: contentWrap

    // Cross extent of the band = the frame's cross dimension (both classicRoot
    ...
    readonly property string style: String(classicConfig.style || "modular")
    readonly property bool fillStyle: BarLayout.isFillStyle(style)
    ...
    // --- strip band geometry (classicRoot == the host content frame) ------------
    ...
    // i.e. the strip thickness is bar.barHeight on horizontal bars and
    // bar.barWidth on vertical bars — both equal to the host content cross
    ...
    readonly property real stripLen: Math.max(0, Math.round((vertical ? height : width)
        * (fillStyle ? 1.0 : (Math.min(100, Math.max(40, classicConfig.widthPercent || 100)) / 100.0))))
    ...
    readonly property bool revealed: !autohideOn || bar.classicAutohideRevealed !== false
    ...
    Item {
        id: contentWrap
        x: classicRoot.vertical ? classicRoot.hideShift : classicRoot.stripStart
        y: classicRoot.vertical ? classicRoot.stripStart : classicRoot.hideShift
        width: classicRoot.vertical ? classicRoot.width : classicRoot.stripLen
        height: classicRoot.vertical ? classicRoot.stripLen : classicRoot.height
        ...
        // clip cuts the overflow (documented behaviour, same as the bar
        // engine's fixed-position zones).
        clip: classicRoot.stripShown

        Behavior on x { enabled: classicRoot.autohideOn; NumberAnimation { duration: 250; easing.type: Easing.OutExpo } }
        Behavior on y { enabled: classicRoot.autohideOn; NumberAnimation { duration: 250; easing.type: Easing.OutExpo } }
        ...
        Rectangle {
            anchors.fill: parent
            visible: classicRoot.stripShown
            radius: classicRoot.stripRadius
            topLeftRadius: classicRoot.vertical
                ? ((bar.position === "left" && classicRoot.edgeFlush) ? 0 : radius)
                : ((bar.position === "top" && classicRoot.edgeFlush) ? 0 : radius)
            topRightRadius: classicRoot.vertical
                ? ((bar.position === "right" && classicRoot.edgeFlush) ? 0 : radius)
                : ((bar.position === "top" && classicRoot.edgeFlush) ? 0 : radius)
            bottomLeftRadius: classicRoot.vertical
                ? ((bar.position === "left" && classicRoot.edgeFlush) ? 0 : radius)
        ...
        // classicConfig object (the section flow models re-evaluate → fresh
        // module Loaders → entrance cascade replays, exactly like a bar
        // config edit).
        property var leftSlots: classicRoot.buildSlots(classicConfig.modules.left)
        property var centerSlots: classicRoot.buildSlots(classicConfig.modules.center)
        property var rightSlots: classicRoot.buildSlots(classicConfig.modules.right)

        // --- LEFT (top) section ----------------------------------------------
        Row {
            visible: !classicRoot.vertical
            x: classicRoot.pad
            y: Math.round((parent.height - height) / 2)
            spacing: classicRoot.itemGap
            Repeater { model: classicRoot.vertical ? [] : contentWrap.leftSlots; delegate: slotDelegate }
        }
        Column {
            visible: classicRoot.vertical
        ...
        Row {
            ...
            y: Math.round((parent.height - height) / 2)
            spacing: classicRoot.itemGap
            Repeater { model: classicRoot.vertical ? [] : contentWrap.centerSlots; delegate: slotDelegate }
        }
        Column {
            visible: classicRoot.vertical
            x: Math.round((parent.width - width) / 2)
        ...
        Row {
            visible: !classicRoot.vertical
            x: parent.width - width - classicRoot.pad
            y: Math.round((parent.height - height) / 2)
            spacing: classicRoot.itemGap
            Repeater { model: classicRoot.vertical ? [] : contentWrap.rightSlots; delegate: slotDelegate }
        }
        Column {
            visible: classicRoot.vertical
            x: Math.round((parent.width - width) / 2)
            y: parent.height - height - classicRoot.pad
            spacing: classicRoot.itemGap
            Repeater { model: classicRoot.vertical ? contentWrap.rightSlots : []; delegate: slotDelegate }
        }
        ...
        // in modular (colors.surface0 — the bar unified tone) and on a flat
        // strip only when classicbar.distinctPills is on (raised segment,
        ...
        Component {
            ...
            Item {
                ...
                readonly property bool horizontal: !classicRoot.vertical
                // Cross extent of the band (strip thickness / width).
                readonly property real bandCross: classicRoot.bandCross
                // Group chrome visibility mirrors TopBar.qml:655
                // (!isSolid || distinctPills): hidden on a plain flat strip.
                readonly property bool groupChromeVisible: groupSlot && (!classicRoot.stripShown || classicRoot.distinctPills)
                ...
                Rectangle {
                    ...
                    visible: slotWrap.groupSlot
                        ...
                        : (classicRoot.stripShown && classicRoot.distinctPills)
                    // Group slabs in modular use the bar unified tone; every
                    // flat-strip slab (group + loose distinct pills) uses the
                    // raised-on-strip tone.
                    color: (slotWrap.groupSlot && !classicRoot.stripShown)
                        ? colors.surface0
                        : Qt.rgba(colors.surface1.r, colors.surface1.g, colors.surface1.b, 0.55)
                    radius: classicRoot.rectRadius
                    x: (classicRoot.vertical && classicRoot.stripShown) ? classicRoot.unitInset : 0
                    y: (!classicRoot.vertical && classicRoot.stripShown) ? classicRoot.unitInset : 0
                    width: slotWrap.width - ((classicRoot.vertical && classicRoot.stripShown) ? classicRoot.unitInset * 2 : 0)
                    height: slotWrap.height - ((!classicRoot.vertical && classicRoot.stripShown) ? classicRoot.unitInset * 2 : 0)
        ...
        // slotIndex replay the bar's entrance cascade.
        Component {
            id: memberDelegate
            Item {
                id: memberWrap
                ...
                width: horizontal ? memberLoader.width : classicRoot.bandCross
                height: horizontal ? classicRoot.bandCross : memberLoader.height
                ...
                Loader {
                    ...
                    Component.onCompleted: {
                        let mod = BarLayout.getModule(modelData.id);
                        if (!mod) return;
                        setSource(Qt.resolvedUrl(mod.component), {
                            "bar": classicRoot.bar,
                            "colors": classicRoot.colors,
                            "zoneReady": true,
    ...
    Item {
        ...
        x: classicRoot.vertical && bar.position === "right" ? classicRoot.width - tabThickness : 0
        y: !classicRoot.vertical && bar.position === "bottom" ? classicRoot.height - tabThickness : 0
        width: classicRoot.vertical ? tabThickness : classicRoot.width
        height: classicRoot.vertical ? classicRoot.height : tabThickness
    }
    HoverHandler {
        id: frameHover
        acceptedDevices: PointerDevice.Mouse
        onHoveredChanged: classicRoot.pollAutohide()
    }
    HoverHandler {
        id: tabHover
        acceptedDevices: PointerDevice.Mouse
    ...
    Timer {
        id: hideTimer
        interval: classicConfig.autohideTimeout
        onTriggered: {
            if (classicRoot.autohideOn) bar.classicAutohideRevealed = false;
        }
    }
    function pollAutohide() {
        if (!autohideOn) return;
        if (bar.classicAutohideRevealed !== false) {
            ...
            hideTimer.stop();
            bar.classicAutohideRevealed = true;
        }
    }
    ...
    onAutohideOnChanged: {
        if (!autohideOn) return;
        bar.classicAutohideRevealed = true;
        Qt.callLater(() => classicRoot.pollAutohide());
    }
    onRevealedChanged: {
        if (classicRoot.revealed && classicRoot.autohideOn) Qt.callLater(() => classicRoot.pollAutohide());
    }
    ...
    Component.onCompleted: {
        if (classicRoot.autohideOn) Qt.callLater(() => classicRoot.pollAutohide());
    }

    // ========================================================================
    // SLOT MODEL BUILDING
    // ========================================================================
    // Flattens one classicbar section list into renderable slots: strings expand
    // through BarLayout.classicTokenModules() (a token becomes several LOOSE
    // modules), arrays become group slots. A replaced classicConfig re-runs the
    ...
    function buildSlots(list) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            if (typeof entry === "string") {
                let expanded = BarLayout.classicTokenModules(entry);
