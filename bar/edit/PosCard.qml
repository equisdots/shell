...
// API intacta: bar / barRef / pos / label / glyph.
Rectangle {
    id: card
    ...
    readonly property bool isOn: barRef && barRef.position === pos
    ...
    MouseArea {
        ...
        onClicked: {
            if (card.bar && card.barRef) {
                card.bar.applyBar(Object.assign({}, card.barRef, { position: card.pos }));
            }
        }
    }
}
